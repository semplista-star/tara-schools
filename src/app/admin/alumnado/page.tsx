...
import { updateConsentStatus } from "@/app/admin/actions";
...
export default async function AlumnadoPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-ink text-lg font-semibold">Alumnado y consentimiento familiar</h2>
        <p className="text-muted text-sm">
          El estado de consentimiento determina si se recogen datos agregados de uso de ese alumno/a. Actualízalo
          cuando la familia entregue o revoque su autorización.
        </p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-muted border-b border-border">
                <th className="px-4 py-2 font-medium">Alumno/a</th>
                <th className="px-4 py-2 font-medium">Estado</th>
                <th className="px-4 py-2 font-medium">Cambiar a</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <td className="px-4 py-3">
                    <form action={updateConsentStatus} className="flex items-center gap-2">
                      ...
                      <label htmlFor={`status-${student.id}`} className="sr-only">
                        Cambiar estado de consentimiento de {student.alias}
                      </label>
                      <select
